...
//    private static TripleStore store = new TripleStore( 500, 0.6f );
    ...
    public T getProxy( K core, Class<?> trait ) throws LogicalTypeInconsistencyException {
        return getProxy( core, trait, null );
    }

    public T getProxy( K core, Class<?> trait, KnowledgeHelper helper ) throws LogicalTypeInconsistencyException {
        ...
        try {
            switch ( mode ) {
                case MAP    :   proxy = konst.newInstance( core, core._getDynamicProperties(), hier.getCode( trait.getName() ), hier.getBottom() );
                    break;
                case TRIPLES:   proxy = konst.newInstance( core, ruleBase.getTripleStore(), getTripleFactory() /*, hier.getCode( trait.getName() ) */ );
                    ((TraitProxy) proxy).setTypeCode( hier.getCode( trait.getName() ) ); // TODO remove when set by constructor
    ...
    private Constructor<T> cacheConstructor( String key, K core, Class<?> trait ) {
        ...
        try {
            Constructor konst;
            switch ( mode ) {
                case MAP    :   konst = proxyClass.getConstructor( core.getClass(), Map.class, BitSet.class, BitSet.class );
                    break;
